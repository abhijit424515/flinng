        ...
        try:
            filtered = get_pareto(
                [
                    d
                    for d in data
                    if d[1] < t and d[0].startswith("R" + compare_by + "@")
                ]
            )
            plt.plot(
                [d[2] for d in filtered],
                [d[3] for d in filtered],
                color=c,
                linestyle=ls,
                marker=mark,
                label=method.upper() + " (" + str(t) + "ms)",
                alpha=0.8,
                markersize=marksize,
            )
        except:
...
plt.title(
    f"{get_dataset_title(dataset)}: Top-{compare_by} Nearest Neighbours",
