...
def _read_hnsw(file_name):
    ...
    with open(file_name, "r") as f:
        while True:
            ...
            record.append(
                (line.split()[0], float(values[0]), float(values[1]), float(values[2]))
            )
    return record
...
def _get_raw_data(file_name, cut):
    if (
        "hnsw" in file_name
        or "falconn" in file_name
        or "inverted" in file_name
        or "faiss" in file_name
        or "groups" in file_name
    ):
...
def get_data_colored(dataset):
    return [
        (method, data, colors[method], markers[method], markersizes[method])
